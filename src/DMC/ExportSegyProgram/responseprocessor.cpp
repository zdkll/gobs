...
bool ResponseProcessor::preProcess(int n, float dt)
{
    ...
    //开辟空间
    m_data = new double[n];
    m_dataComplex = (std::complex<double>*) fftw_malloc(sizeof(std::complex<double>) * m_outNum);

    //构造好fftw_plan
    m_r2cPlan = fftw_plan_dft_r2c_1d(m_inNum,m_data,reinterpret_cast<fftw_complex*>(m_dataComplex),FFTW_MEASURE);
    m_c2rPlan = fftw_plan_dft_c2r_1d(m_inNum,reinterpret_cast<fftw_complex*>(m_dataComplex),m_data,FFTW_MEASURE);

    //构造Hs
    m_hs =  new std::complex<double>[m_nf2 - m_nf1 + 1];
    int nh = 0;
    complex<double> fac1(359.4,0);
    complex<double> fac2(27.18,0);
    complex<double> fac3(24.72,0);
    complex<double> fac4(10.42,0);
    complex<double> fac5(358.7,0);
    complex<double> fac6(40.7,0);
    for(int i=m_nf1;i<=m_nf2;i++){
        complex<double> s(0, 2*PI*(df*i));

        m_hs[nh] = ((s+fac1)*(s+fac2)*(s+fac3)*(s+fac4))/(497.0033*(s*s)*(s+fac5)*(s+fac6)) ;
...
void ResponseProcessor::fftwResponseProcess(double *data)
{
    ...
    if(n1>0)
        memset(m_dataComplex,0,(n1)*sizeof(std::complex<double>));
    if(m_outNum-1>m_nf2){
        memset(m_dataComplex+m_nf2+1,0,(m_outNum-m_nf2-1)*sizeof(std::complex<double>));
    }
